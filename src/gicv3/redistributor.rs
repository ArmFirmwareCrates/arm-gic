...
/// Use `GicRedistributorContext::ppi_count` to determine the const parameter based on the PPI
/// count.
///
/// ```
/// use arm_gic::gicv3::GicRedistributorContext;
///
/// let context = GicRedistributorContext::<{ GicRedistributorContext::ppi_count(96) }>::default();
/// ```
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GicRedistributorContext<const MAX_PPI_COUNT_32: usize> {
    propbaser: u64,
    pendbaser: u64,
    ctlr: GicrCtlr,
    nsacr: u32,
    // These can be simplified once feature(generic_const_exprs) is stabilized
    igroupr: [[u32; Sgi::IGROUPR_BITS]; MAX_PPI_COUNT_32],
    isenabler: [[u32; Sgi::ISENABLER_BITS]; MAX_PPI_COUNT_32],
    ispendr: [[u32; Sgi::ISPENDR_BITS]; MAX_PPI_COUNT_32],
    isactiver: [[u32; Sgi::ISACTIVER_BITS]; MAX_PPI_COUNT_32],
    igrpmodr: [[u32; Sgi::IGRPMODR_BITS]; MAX_PPI_COUNT_32],
    icfgr: [[u32; Sgi::ICFGR_BITS]; MAX_PPI_COUNT_32],
    ipriorityr: [[u32; Sgi::IPRIORITYR_BITS]; MAX_PPI_COUNT_32],
}

impl GicRedistributorContext<0> {
    /// Calculates `MAX_PPI_COUNT_32` const parameter of `GicRedistributorContext` based on the PPI count.
    pub const fn ppi_count(ppi_count: usize) -> usize {
        ppi_count.div_ceil(32)
    }
}

impl<const MAX_PPI_COUNT_32: usize> GicRedistributorContext<MAX_PPI_COUNT_32> {
    const MAX_PPI_COUNT: usize = MAX_PPI_COUNT_32 * 32;
...
impl<const MAX_PPI_COUNT_32: usize> Default for GicRedistributorContext<MAX_PPI_COUNT_32> {
    fn default() -> Self {
        Self::new()
    }
}
...
impl<'a> GicRedistributor<'a> {
    ...
    pub fn restore<const N: usize>(
        ...
        let ppi_count = Self::min_count(
            self.ppi_count(),
            GicRedistributorContext::<N>::MAX_PPI_COUNT,
        )?;
...
mod tests {
    ...
    fn context_size() {
        assert_eq!(
            ...
            size_of::<GicRedistributorContext::<{ GicRedistributorContext::ppi_count(32) }>>()
        );

        assert_eq!(
            (24usize + 64 * 15 / 8).next_multiple_of(size_of::<usize>()),
            size_of::<GicRedistributorContext::<{ GicRedistributorContext::ppi_count(64) }>>()
        );

        assert_eq!(
            (24usize + 96 * 15 / 8).next_multiple_of(size_of::<usize>()),
            size_of::<GicRedistributorContext::<{ GicRedistributorContext::ppi_count(96) }>>()
        );
    }
    ...
    fn save_restore() {
        let mut context =
            GicRedistributorContext::<{ GicRedistributorContext::ppi_count(32) }>::new();
