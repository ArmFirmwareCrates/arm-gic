...
macro_rules! set_regs {
    ($regs:expr, $reg:ident, $int_count:expr, $bits_per_int:expr, $value:expr) => {
        let reg_count = register_count!($int_count, $bits_per_int, $value);
        for i in 0..reg_count {
            field!($regs, $reg).get(i).unwrap().write($value);
        }
    };
    ($regs:expr, $reg:ident, $int_start:expr, $int_end:expr, $bits_per_int:expr, $value:expr) => {
        let reg_start = register_count!($int_start, $bits_per_int, $value);
        let reg_end = register_count!($int_end, $bits_per_int, $value);
...
impl<'a> GicRedistributor<'a> {
    ...
    pub fn configure_default_settings(&mut self) {
        ...
        let mut sgi = field!(self.regs, sgi);
        set_regs!(sgi, igroupr, ppi_count, Sgi::IGROUPR_BITS, 0xffff_ffffu32);
    ...
    /// Sets the interrupt priority of the given interrupt ID.
    ...
    pub fn restore(&mut self, context: &GicRedistributorContext) {
        ...
        restore_regs!(context, sgi, igroupr, ppi_count, Sgi::IGROUPR_BITS);
        restore_regs!(context, sgi, igrpmodr, ppi_count, Sgi::IGRPMODR_BITS);
        restore_regs!(context, sgi, ipriorityr, ppi_count, Sgi::IPRIORITY_BITS);
        restore_regs!(context, sgi, icfgr, ppi_count, Sgi::ICFGR_BITS);
        restore_reg!(context, sgi, nsacr);

        // Restore after group and priorities are set.
        restore_regs!(context, sgi, ispendr, ppi_count, Sgi::ISPENDR_BITS);
        restore_regs!(context, sgi, isactiver, ppi_count, Sgi::ISACTIVER_BITS);
        ...
        let mut sgi = field!(self.regs, sgi);
        restore_regs!(context, sgi, isenabler, ppi_count, Sgi::ISENABLER_BITS);
    ...
    pub fn save(&self, context: &mut GicRedistributorContext) {
        ...
        save_regs!(context, sgi, igroupr, ppi_count, Sgi::IGROUPR_BITS);
        save_regs!(context, sgi, isenabler, ppi_count, Sgi::ISENABLER_BITS);
        save_regs!(context, sgi, ispendr, ppi_count, Sgi::ISPENDR_BITS);
        save_regs!(context, sgi, isactiver, ppi_count, Sgi::ISACTIVER_BITS);
        save_regs!(context, sgi, igrpmodr, ppi_count, Sgi::IGRPMODR_BITS);
        save_regs!(context, sgi, icfgr, ppi_count, Sgi::ICFGR_BITS);
        save_regs!(context, sgi, ipriorityr, ppi_count, Sgi::IPRIORITY_BITS);
